  ...
  class NestedParamsEncoder < Crest::ParamsEncoder
    ...
    def encode(params : Hash) : String
      HTTP::Params.build do |form|
        NestedParamsEncoder.flatten_params(params).each do |name, value|
    ...
    # # => [{"key1[key2]", "123"}]
    # ```
    def self.flatten_params(object : Hash, parent_key = nil) : Array(Tuple(String, ParamsValue))
    ...
    # ```
    def self.flatten_params(object : Array, parent_key = nil) : Array(Tuple(String, ParamsValue))
      object.reduce([] of Tuple(String, ParamsValue)) do |memo, item|
